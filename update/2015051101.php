...
if (self::$settings->checkNetworkActivation ()) {
	$oldGlobalOptions = get_site_option ( 'wp-mvmcloud_global-settings', array () );
	delete_site_option('wp-mvmcloud_global-settings');
} else {
	$oldGlobalOptions = get_option ( 'wp-mvmcloud_global-settings', array () );
	delete_option('wp-mvmcloud_global-settings');
}

$oldOptions = get_option ( 'wp-mvmcloud_settings', array () );
delete_option('wp-mvmcloud_settings');
...
if (self::$settings->checkNetworkActivation ()) {
	...
	if (is_array($aryBlogs))
		foreach ($aryBlogs as $aryBlog) {
            $oldOptions = get_blog_option ( $aryBlog['blog_id'], 'wp-mvmcloud_settings', array () );
			if (!$this->isConfigured())
				foreach ( $oldOptions as $key => $value )
					self::$settings->setOption ( $key, $value, $aryBlog['blog_id'] );
			delete_blog_option($aryBlog['blog_id'], 'wp-mvmcloud_settings');
		}
}
