...
// set to true if the target mvmcloud server has a ssl certificate that will fail verification, like when testing.
$NO_VERIFY_SSL = false;
...
// Restore $path
$path=$wp_mvmcloud_proxy_path;

// -----
// Important: read the instructions in README.md or at:
// https://github.com/mvmcloud-org/tracker-proxy#mvmcloud-tracker-proxy
// -----
if (! isset($MVMCLOUD_URL) && isset($MVMCLOUD_URL)) {
    // FOR BC
    $MVMCLOUD_URL = $MVMCLOUD_URL;
}

// Edit the line below, and replace http://your-mvmcloud-domain.example.org/mvmcloud/
// with your MVMCloud Analytics URL ending with a slash.
// This URL will never be revealed to visitors or search engines.
if (! isset($MVMCLOUD_URL)) {
    $MVMCLOUD_URL = 'http://your-mvmcloud-domain.example.org/mvmcloud/';
}
...
// Maximum time, in seconds, to wait for the MVMCloud Analytics server to return the 1*1 GIF
if (! isset($timeout)) {
    $timeout = 5;
}

// The HTTP User-Agent to set in the request sent to MVMCloud Analytics Tracking API
if (empty($user_agent)) {
...
// 1) MVMCLOUD.JS PROXY: No _GET parameter, we serve the JS file; or we serve a requested js file
if ((empty($_GET) && empty($_POST)) || (isset($filerequest) && substr($filerequest, -3) === '.js')) {
    ...
    // Re-download the mvmcloud.js once a day maximum
    ...
    if (!empty($modifiedSince) && $modifiedSince > $lastModified) {
        ...
        // Silent fail: hide Warning in 'mvmcloud.js' response
        if (empty($_GET) && empty($_POST)) {
            if ($path !== 'mvmcloud.php') {
                $jsPath = 'mvmcloud.js'; // for BC eg in case user uses an older version of MVMCloud Analytics
            } else {
                $jsPath = 'mvmcloud.js';
            }
            list($content, $httpStatus) = getHttpContentAndStatus($MVMCLOUD_URL . $jsPath, $timeout, $user_agent);
        } else {
            list($content, $httpStatus) = getHttpContentAndStatus($MVMCLOUD_URL . $filerequest, $timeout, $user_agent);
        }
        if ($mvmcloudJs = $content) {
            echo $mvmcloudJs;
        } else {
            echo '/* there was an error loading mvmcloud.js */';
        }
    }
    exit;
}
...
// 2) MVMCLOUD.PHP PROXY: GET parameters found, this is a tracking request, we redirect it to MVMCloud Analytics
if (strpos($path, '?') === false) {
    $path = $path . '?';
}

$extraQueryParams = array();
if (strpos($path, 'mvmcloud.php') === 0 || strpos($path, 'mvmcloud.php') === 0) {
    $extraQueryParams = array(
        'cip' => getVisitIp(),
...
function sanitizeContent($content)
{
    ...
    $mvmcloudHost = parse_url($MVMCLOUD_URL, PHP_URL_HOST);
    $proxyHost = parse_url($PROXY_URL, PHP_URL_HOST);

    $content = str_replace($TOKEN_AUTH, '<token>', $content);
    $content = str_replace($MVMCLOUD_URL, $PROXY_URL, $content);
    $content = str_replace($mvmcloudHost, $proxyHost, $content);

    if(isset($VALID_FILES)) {
        foreach($VALID_FILES as $filepath) {
            // replace file paths to match the proxy and discard cb
            $content = preg_replace('^' . $filepath . '(\?cb\=[a-z0-9]*)?^', $PROXY_URL . 'mvmcloud-proxy.php?file=' . $filepath, $content);
        }
    }
...
function getHttpContentAndStatus($url, $timeout, $user_agent)
{
    ...
    // NOTE: any changes made to MVMCloud Analytics\Plugins\PrivacyManager\DoNotTrackHeaderChecker must be made here as well
