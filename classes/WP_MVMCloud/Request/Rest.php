<?php
	...
	class Rest extends \WP_MVMCloud\Request {

		protected function request($id) {
			$count = 0;
			if (self::$settings->getGlobalOption('mvmcloud_mode') == 'http')
				$url = self::$settings->getGlobalOption('mvmcloud_url');
			else if (self::$settings->getGlobalOption('mvmcloud_mode') == 'cloud')
				$url = 'https://'.self::$settings->getGlobalOption('mvmcloud_user').'.innocraft.cloud/';
			else $url = 'https://'.self::$settings->getGlobalOption('mvmcloud_user').'.mvmcloud.cloud/';
		...
		private function curl($id, $url, $params) {
			if (self::$settings->getGlobalOption('http_method')=='post') {
				...
				curl_setopt($c, CURLOPT_POSTFIELDS, $params.'&token_auth='.self::$settings->getGlobalOption('mvmcloud_token'));
			} else $c = curl_init($url.'?'.$params.'&token_auth='.self::$settings->getGlobalOption('mvmcloud_token'));
			...
			curl_setopt($c, CURLOPT_USERAGENT, self::$settings->getGlobalOption('mvmcloud_useragent')=='php'?ini_get('user_agent'):self::$settings->getGlobalOption('mvmcloud_useragent_string'));
			curl_setopt($c, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($c, CURLOPT_HEADER, $GLOBALS ['wp-mvmcloud_debug'] );
		...
		private function fopen($id, $url, $params) {
			...
			if (self::$settings->getGlobalOption('http_method')=='post') {
				...
				$contextDefinition['http']['content'] = $params.'&token_auth='.self::$settings->getGlobalOption('mvmcloud_token');
			} else $fullUrl = $url.'?'.$params.'&token_auth='.self::$settings->getGlobalOption('mvmcloud_token');
